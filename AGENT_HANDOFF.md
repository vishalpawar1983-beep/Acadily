...
Use `scripts/deploy-production.sh` (bash/WSL) or run the steps manually:
...
SSHPASS='zQ>iaRo' sshpass -e ssh -o StrictHostKeyChecking=no \
  -o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedKeyTypes=+ssh-rsa root@66.116.207.89 << 'REMOTE'
...
docker run -d --name flex-academy-api --restart unless-stopped --memory 800m \
  ...
  -e USER_EMAIL=visualmediatechnology@gmail.com -e USER_PASSWORD=vdjojwhjxvhsgijt \
...
# 5. Local cleanup + smoke test
rm /tmp/flex-academy-api-amd64.tar.gz
...
**Recovery**: Start Docker Desktop, then: `until docker info > /dev/null 2>&1; do sleep 3; done`
...
| Page | ID | What |
|---|---|---|
| Common Pitfalls & Gotchas | 338853889 | All numbered pitfalls (currently #1-22) |
...
6. KB memory: update project memory if a new persistent fact emerged
